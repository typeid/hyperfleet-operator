...
5. **Developer reviews** the diff and sets the appropriate boundary 2 markers on new fields

```
go.mod (bump HyperShift)
    → make generate-passthrough
        → api/v1alpha1/hostedcluster_passthrough_types.go (updated)
        → api/v1alpha1/nodepool_passthrough_types.go (updated)
    → developer annotates new fields (visibility, write mode, gates)
    → make manifests + make openapi
        → CRD YAML + per-feature-set variants
        → OpenAPI spec + field metadata registry
```
...
This boundary applies to **all** HyperFleet CRDs (wrappers and native). Three Go markers on the CRD types control what reaches the customer-facing API. Codegen reads these markers and produces the OpenAPI spec, per-feature-set CRD variants, and a field metadata registry.
...
Controls whether customers can set or change a field. A codegen step reads these markers and produces a **field metadata registry** (generated Go code) that the Platform API imports and enforces on every mutation.
...
The CRD also carries CEL immutability rules (`self.region == oldSelf.region`) as defense-in-depth. Write modes are also documented in the OpenAPI spec via `x-write-mode` extensions.
...
**Example**: A customer sends `POST /clusters` with:
...
```json
{
  ...
  "hostedCluster": {
    "release": { "image": "4.16.3" },
    "etcd": { "storageSize": "16Gi" }
  }
}
```

| Field | Registry lookup | Result |
| ----- | --------------- | ------ |
| `spec.name` | immutable, create | Allowed (immutable fields are settable on create) |
| `spec.region` | immutable, create | Allowed |
| `spec.deleteProtection` | mutable | Allowed |
| `spec.hostedCluster.release` | immutable, create | Allowed |
| `spec.hostedCluster.etcd` | gate: `HyperFleetEtcdConfig` | Check customer entitlements. Allowed if gate enabled, 400 if not |

After validation passes, the Platform API enriches the CR with service-set fields (`accountId`, `creatorARN`, etc.) and creates it.

### Type Conversion (CRD ↔ REST)

The REST types (generated by oapi-codegen from the OpenAPI spec) only have visible fields, because `+k8s:openapi-gen=false` fields were excluded from the OpenAPI spec before oapi-codegen ran. Codegen produces bidirectional mapping functions between CRD types and REST types. Since the two type sets share field names, no hand-written conversion code is needed.

**Read path** (CRD → REST): hidden fields are dropped structurally because they don't exist on the target type.
